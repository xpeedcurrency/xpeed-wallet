...
import 'package:xpeed_wallet_flutter/network/model/response/account_balance_item.dart';
import 'package:xpeed_wallet_flutter/ui/transfer/transfer_manual_entry_sheet.dart';
...
class AppTransferOverviewSheet {
  ...
  Future<List<String>> getAccountsFromSeed(BuildContext context, String seed) async {
    ...
    for (int i = 0; i < NUM_SWEEP; i++) {
      privKey = await _xpeedUtil.seedToPrivateInIsolate(seed, i);
      address = await _xpeedUtil.seedToAddressInIsolate(seed, i);
    ...
    address = NanoAccounts.createAccount(
        NanoAccountType.XPD, NanoKeys.createPublicKey(seed));
    if (address != StateContainer.of(context).wallet.address) {
      privKeyBalanceMap.putIfAbsent(
