...
class DBHelper{
  ...
  XpeedUtil _xpeedUtil;

  DBHelper() {
    _xpeedUtil = XpeedUtil();
  }
  ...
  initDb() async {
    ...
    String path = join(documentsDirectory.path, "xpeedwallet.db");
  ...
  Future<Contact> getContactWithAddress(String address) async {
    var dbClient = await db;
    List<Map> list = await dbClient.rawQuery('SELECT * FROM Contacts WHERE address like \'%${address.replaceAll("xpd_", "")}\'');
    if (list.length > 0) {
  ...
  Future<bool> contactExistsWithAddress(String address) async {
    var dbClient = await db;
    int count = Sqflite.firstIntValue(await dbClient.rawQuery('SELECT count(*) FROM Contacts WHERE lower(address) like \'%${address.replaceAll("xpd_", "")}\''));
    return count > 0;
  }
  ...
  Future<List<Account>> getAccounts() async {
    ...
    for (int i = 0; i < list.length; i++) {
      String address;
      address = await _xpeedUtil.seedToAddressInIsolate(await sl.get<Vault>().getSeed(), list[i]["acct_index"]);
  ...
  Future<List<Account>> getRecentlyUsedAccounts({int limit = 2}) async {
    ...
    List<Account> accounts = new List();
    for (int i = 0; i < list.length; i++) {
      accounts.add(Account(id: list[i]["id"], name: list[i]["name"], index: list[i]["acct_index"], lastAccess: list[i]["last_accessed"], selected: list[i]["selected"] == 1 ? true : false, address: await _xpeedUtil.seedToAddressInIsolate(await sl.get<Vault>().getSeed(), list[i]["acct_index"]), balance: list[i]["balance"]));
    }
    return accounts;
  }
  ...
  Future<Account> addAccount({String nameBuilder}) async {
    ...
    await dbClient.transaction((Transaction txn) async {
      ...
      account = Account(index: nextIndex, name:nextName, lastAccess: 0, selected: false, address: await _xpeedUtil.seedToAddressInIsolate(await sl.get<Vault>().getSeed(), nextIndex));
  ...
  Future<Account> getSelectedAccount() async {
    ...
    Account account = Account(id: list[0]["id"], name: list[0]["name"], index: list[0]["acct_index"], selected: true, lastAccess: list[0]["last_accessed"], balance: list[0]["balance"],  address: await _xpeedUtil.seedToAddressInIsolate(await sl.get<Vault>().getSeed(), list[0]["acct_index"]));
    return account;
  }
